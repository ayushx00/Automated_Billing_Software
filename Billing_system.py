...
class Bill_App:
    def __init__(self, root):
        ...
        title.pack(fill=X)
          ...
          # Initialize the bill_no as a StringVar
        self.bill_no = StringVar()
        
        # Now you can safely use self.bill_no.set
        self.bill_no.set(str(0))
...
#=========billArea=================================================
    ...
    def save_bill(self):
        ...
        try:
            ...
            mb.showinfo("Debug", f"Base dir: {base_dir}")  # Show base directory

            bills_dir = os.path.join(base_dir, "bills")

            if not os.path.exists(bills_dir):
                os.makedirs(bills_dir)
                mb.showinfo("Debug", "Created 'bills' folder")

            file_path = os.path.join(bills_dir, str(self.bill_no.get()) + ".txt")
            with open(file_path, "w") as f:
                f.write("Test bill content")

            mb.showinfo("Success", f"Bill saved at:\n{file_path}")
        ...
        try:
            # Existing code to save bill to text file...
            # (Assume self.bill_no and self.txtarea exist)

            # ✅ Now, insert into SQLite database
            conn = sqlite3.connect("billing.db")  # Creates DB file if it doesn't exist
            cursor = conn.cursor()
            ...
            # Insert the bill
            cursor.execute('''
                INSERT INTO bills (bill_no, content)
                VALUES (?, ?)
            ''', (self.bill_no.get(), self.txtarea.get("1.0", "end-1c")))

            conn.commit()
            conn.close()

            print("✅ Bill stored in database.")

        except Exception as e:
            print("❌ Error while saving to database:", str(e))
